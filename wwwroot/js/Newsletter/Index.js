...
function AddUp(success_text, error_text, place) {
    ...
    co.Articles.AddUp({
        ...
        if (result.success) {
            var list = [];
            list.push(($.Deferred()).resolve());
            $(".image_upload").each(function () {
                if ($(this).find(".img_input").data("file") != null && $(this).find(".img_input").data("file").File != null && $(this).find(".img_input").data("file").Id == 0) {
                    const _dfr = $.Deferred();
